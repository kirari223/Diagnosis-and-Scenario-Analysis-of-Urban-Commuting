...
print('--- Phase 1: json.loads Attempt ---')
...
try:
    ...
    start, end = max(0, e.lineno - 4), min(len(lines), e.lineno + 3)
    for i in range(start, end):
        p = '>>>' if i == e.lineno - 1 else '   '
        print(f'{i+1:4}: {p} {lines[i]}')
print('\n--- Phase 2: Lines 145-165 and 300-320 ---')
lines = content.splitlines()
print('Lines 145-165:')
for i in range(144, min(165, len(lines))): print(f'{i+1:4}: {lines[i]}')
print('\nLines 300-320:')
for i in range(299, min(320, len(lines))): print(f'{i+1:4}: {lines[i]}')
print('\n--- Phase 3: Suspicious Double Quote Patterns ---')
for i, line in enumerate(lines):
    u = re.findall(r'(?<!\\)"', line)
    if len(u) > 2 and len(u) % 2 != 0: print(f'Line {i+1}: Potential unescaped quote -> {line.strip()}')
